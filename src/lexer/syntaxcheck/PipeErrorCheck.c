...
int	run_miss_arg(t_lexlist *lex_list)
{
	...
	if (!ptr)
		return (1);
	create_lexlist(ptr, &lex_list); // lexlist oluştur
	classify(lex_list); // lexlisti sınıflandır
	str_addchar(&g_core.cmd, ' '); // cmd'nin sonuna ' ' karakterini ekler
	own_strjoin(&g_core.cmd, ptr); // cmd'nin sonuna ptr'yi ekler
	free(ptr); //ptr'i free eder
	if (str_compare(lex_list->next->content, PIPE)) // next'de pipe varsa
		return (-1);
