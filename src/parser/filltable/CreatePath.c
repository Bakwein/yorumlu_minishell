...
char	**create_path(t_lexlist *lex_list)
{
	...
	count = 0;
	while (lex_list && lex_list->type != SIGN_PIPE) // lex_list NULL değilse ve lex_list->type SIGN_PIPE değilse
	{
		if (lex_list->type == TEXT && lex_list->content) // lex_list->type TEXT ve lex_list->content NULL değilse
			count++; // count arttırılır
		else // lex_list->type TEXT değilse veya lex_list->content NULL ise(metachar)
		{
